...
def find_matches(bert_outputs: Tensor,
    ...
    n_batch, n_id, n_source = sparse_spans(n_spanss, offsets)
    v_batch, v_id, v_source = sparse_spans(v_spans, offsets)
    ...
    vn_mask: Tensor = ...                           # (num_verbs, num_nouns)    -- verb to noun attention
    x_atn: Tensor = ...                             # (num_verbs, num_nouns)    -- verb to noun attention
    x_atn[vn_mask.eq(0)] = -1e10
    return x_atn.softmax(dim=-1)


def sparse_spans(spanss: list[list[list[int]]], offsets: Sequence[int]) -> tuple[Tensor, Tensor, Tensor]:
    ...
    return torch.tensor(batch_idx), torch.cat(token_idx), torch.cat(spans)
